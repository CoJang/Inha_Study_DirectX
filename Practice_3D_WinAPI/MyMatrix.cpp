...
{
	MyVector3 lookat = (target - eye).Normalize();
	MyVector3 right = MyVector3::Cross(up, lookat).Normalize();
	MyVector3 newUp = MyVector3::Cross(lookat, right).Normalize();
	
	MyMatrix viewMat = MyMatrix::Identity(4);
	...
	viewMat[3][2] = -MyVector3::Dot(lookat, eye);
