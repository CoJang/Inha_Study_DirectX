...
void cObjLoader::LoadMtlLib(const char* folder, const char* file)
{
	...
	while (!feof(SrcFile))
	{
		...
		else if (buff[0] == 'm' || buff[1] == 'm')
		{
			...
			D3DXCreateTextureFromFileA(g_pDevice, Path.c_str(), &texture);
...
LPD3DXMESH cObjLoader::LoadMeshOBJ(OUT vector<cMtlTex*> & vecMtlTex, const char* folder, const char* file)
{
	...
	D3DXCreateMeshFVF(vecVertex.size() / 3, vecVertex.size(), D3DXMESH_MANAGED, PNT_VERTEX::FVF, g_pDevice, &pMesh);
	PNT_VERTEX * pV = NULL;
	pMesh->LockVertexBuffer(0, (LPVOID*)&pV);
	memcpy(pV, &vecVertex[0], vecVertex.size() * sizeof(PNT_VERTEX));
