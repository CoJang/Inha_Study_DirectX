...
void cCamera::Setup(D3DXVECTOR3* pvTarget)
{
	...
	D3DXMatrixPerspectiveFovLH(&matProj, D3DX_PI / 4.0F,
	                           rc.right / static_cast<float>(rc.bottom), 1.0f, 1000.0f);
...
void cCamera::WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_MOUSEMOVE:
		if (m_isLButtonDown)
		{
			...
			float fDeltaX = static_cast<float>(ptCurMouse.x) - m_ptPrevMouse.x;
			float fDeltaY = static_cast<float>(ptCurMouse.y) - m_ptPrevMouse.y;
